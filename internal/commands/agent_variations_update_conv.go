...
func ConvertAgentVariationsUpdate(cmd *cli.Command, out *AgentVariationsUpdateConversion) error {
	...
	if cmd.IsSet("assignment") {
		if err := _body.applyDocItems("assignment", []string{"spec", "assignments"}, cmd.StringSlice("assignment")); err != nil {
			return cli.Exit(err.Error(), 2)
		}
	}
	if cmd.IsSet("memory-layer-assignment") {
		if err := _body.applyShorthandItems("memory-layer-assignment", []string{"spec", "memoryLayerAssignments"}, cmd.StringSlice("memory-layer-assignment"), shorthandSpec{Fields: []shorthandField{{Wire: "memoryLayerId", Key: "memory-layer-id", Kind: scalarString, Enum: nil, Required: true}, {Wire: "position", Key: "position", Kind: scalarInt, Enum: nil, Required: true}}}); err != nil {
			return cli.Exit(err.Error(), 2)
		}
	}
