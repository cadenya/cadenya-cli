...
// WorkspaceAdminUpdateConversion is the typed request produced from one urfave command.
type WorkspaceAdminUpdateConversion struct {
	Params sdk.WorkspaceAdminUpdateParams
	Body   any
}

// ConvertWorkspaceAdminUpdate reads urfave values according to Redwood's operation IR.
func ConvertWorkspaceAdminUpdate(cmd *cli.Command, out *WorkspaceAdminUpdateConversion) error {
	...
	if err := _body.finish(_schema, map[string]string{"metadata": "--metadata", "metadata.name": "--name", "metadata.externalId": "--external-id", "metadata.labels": "--label", "spec": "--spec", "spec.description": "--description", "updateMask": "--update-mask"}, _strict); err != nil {
		return cli.Exit(err.Error(), 2)
	}
	_ = _rawBody
