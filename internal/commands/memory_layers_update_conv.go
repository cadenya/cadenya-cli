...
// MemoryLayersUpdateConversion is the typed request produced from one urfave command.
type MemoryLayersUpdateConversion struct {
	Params sdk.MemoryLayerUpdateParams
	Body   any
}

// ConvertMemoryLayersUpdate reads urfave values according to Redwood's operation IR.
func ConvertMemoryLayersUpdate(cmd *cli.Command, out *MemoryLayersUpdateConversion) error {
	...
	if err := _body.finish(_schema, map[string]string{"metadata": "--metadata", "metadata.name": "--name", "metadata.externalId": "--external-id", "metadata.labels": "--label", "spec": "--spec", "spec.type": "--type", "spec.description": "--description", "updateMask": "--update-mask"}, _strict); err != nil {
		return cli.Exit(err.Error(), 2)
	}
	_ = _rawBody
