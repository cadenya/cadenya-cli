		...
		Commands: []*cli.Command{
			...
			},
			{
				...
				Flags: []cli.Flag{
					...
					&cli.StringFlag{Name: "default-variation", Usage: "Optional default variation to add to the agent on create. Its spec accepts assignments and memory_layer_assignments using the same atomic validation as…. YAML/JSON document (literal, @path, or - for stdin).", TakesFile: true},
					...
					&cli.StringSliceFlag{Name: "default-variation-assignment", Usage: "Complete set of assigned tools, tool sets, sub-agents, and agent pools. Order has no meaning. Duplicate (target kind, canonical target ID) pairs are collapsed.…. One YAML/JSON document per occurrence (literal, @path, or -)."},
					&cli.StringSliceFlag{Name: "default-variation-memory-layer-assignment", Usage: "Complete baseline memory cascade, returned in ascending position. At most 10 whole layers; system-managed layers cannot be assigned. Duplicate layer IDs (after…. key=value,... over memory-layer-id, position (repeatable; or a document)."},
				...
				Action: func(ctx context.Context, cmd *cli.Command) error {
					...
					_stdinInputs = append(_stdinInputs, cmd.StringSlice("default-variation-assignment")...)
					_stdinInputs = append(_stdinInputs, cmd.StringSlice("default-variation-memory-layer-assignment")...)
