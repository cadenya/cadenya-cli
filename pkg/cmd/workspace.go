...
var workspacesCreate = requestflag.WithInnerFlags(cli.Command{
	Name:    "create",
	Usage:   "Creates a new workspace in the current account. Requires the admin role.",
	Suggest: true,
	Flags: []cli.Flag{
...
var workspacesRetrieve = cli.Command{
	Name:    "retrieve",
	Usage:   "Retrieves a workspace by ID from the current account.",
	Suggest: true,
	Flags: []cli.Flag{
...
var workspacesDelete = cli.Command{
	Name:    "delete",
	Usage:   "Archives a workspace. This is a soft delete: the workspace is retained but any\nsubsequent request scoped to it returns a permission error. Requires the admin\nrole.",
	Suggest: true,
	Flags: []cli.Flag{
		&requestflag.Flag[string]{
			Name:      "id",
...
var workspacesGetCurrent = cli.Command{
	Name:            "get-current",
	...
	Suggest:         true,
	Flags:           []cli.Flag{},
	Action:          handleWorkspacesGetCurrent,
	HideHelpCommand: true,
}
...
func handleWorkspacesCreate(ctx context.Context, cmd *cli.Command) error {
	...
	params := cadenya.WorkspaceNewParams{}
...
func handleWorkspacesRetrieve(ctx context.Context, cmd *cli.Command) error {
	...
	var res []byte
	options = append(options, option.WithResponseBodyInto(&res))
	_, err = client.Workspaces.Get(ctx, cmd.Value("id").(string), options...)
	if err != nil {
		return err
	}
...
func handleWorkspacesDelete(ctx context.Context, cmd *cli.Command) error {
	...
	return client.Workspaces.Delete(ctx, cmd.Value("id").(string), options...)
}

func handleWorkspacesGetCurrent(ctx context.Context, cmd *cli.Command) error {
	client := cadenya.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	...
	var res []byte
	options = append(options, option.WithResponseBodyInto(&res))
	_, err = client.Workspaces.GetCurrent(ctx, options...)
	if err != nil {
		return err
	}
