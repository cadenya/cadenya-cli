...
func handleWorkspacesList(ctx context.Context, cmd *cli.Command) error {
	...
	if format == "raw" {
		...
		return ShowJSON(os.Stdout, os.Stderr, "workspaces list", obj, format, explicitFormat, transform)
	} else {
		...
		return ShowJSONIterator(os.Stdout, os.Stderr, "workspaces list", iter, format, explicitFormat, transform, maxItems)
	}
}
...
func handleWorkspacesGet(ctx context.Context, cmd *cli.Command) error {
	...
	explicitFormat := cmd.Root().IsSet("format")
	transform := cmd.Root().String("transform")
	return ShowJSON(os.Stdout, os.Stderr, "workspaces get", obj, format, explicitFormat, transform)
}
