		...
		Commands: []*cli.Command{
			...
			},
			{
				...
				Flags: []cli.Flag{
					...
					&cli.StringFlag{Name: "tenant", Usage: "Required tenant assertion — the customer's organization identifier. Upserts the tenant record in the workspace. Every conversation created through this…. YAML/JSON document (literal, @path, or - for stdin).", TakesFile: true},
					&cli.StringFlag{Name: "tenant-id", Usage: "Required. The tenant identifier in the customer's namespace (e.g. \"acme-corp\"). Stored as the tenant record's external_id; stable across requests."},
					...
					&cli.StringFlag{Name: "subject", Usage: "Required subject assertion — the visitor's ID within the tenant. Sessions with the same tenant and subject share conversation history on the same widget and…. YAML/JSON document (literal, @path, or - for stdin).", TakesFile: true},
					&cli.StringFlag{Name: "subject-id", Usage: "Required. The subject identifier in the customer's namespace (e.g. their user id). Stored as the subject record's external_id; unique within the tenant."},
